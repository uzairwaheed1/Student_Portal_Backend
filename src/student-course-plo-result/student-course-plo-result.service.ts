...
export class StudentCoursePloResultService {
  private readonly logger = new Logger(StudentCoursePloResultService.name);
  ...
  async uploadBulkPloResults(uploadDto: UploadCoursePloResultsDto, facultyId: number) {
    ...
    try {
      ...
      // Note: PLO cache is automatically updated via database triggers
      // No need to manually trigger calculation - database handles it automatically
